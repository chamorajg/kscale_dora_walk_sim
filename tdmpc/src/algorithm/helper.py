...
class Episode(object):
    ...
    def __init__(self, cfg, init_obs):
        ...
        if cfg.modality in {"pixels", "state"}:
            ...
            self.next_obses = torch.empty(
                (cfg.num_envs, cfg.episode_length, *init_obs.shape[1:]),
                dtype=dtype,
                device=self.device,
            )
            ...
            for k, v in init_obs.items():
                ...
                self.next_obses[k] = torch.empty(
                    (cfg.num_envs, cfg.episode_length, *v.shape[1:]), dtype=dtype, device=self.device
                )
        ...
        self.actions = torch.empty(
            (cfg.num_envs, cfg.episode_length, cfg.action_dim),
            dtype=torch.float32,
            device=self.device,
        )
        self.rewards = torch.empty(
            (cfg.num_envs, cfg.episode_length,), dtype=torch.float32, device=self.device
        )
        self.dones = torch.empty(
            (cfg.num_envs, cfg.episode_length,), dtype=torch.bool, device=self.device
        )
    ...
    def add(self, obs, action, reward, done, success=False):
        if isinstance(obs, dict):
            for k, v in obs.items():
                if self._idx == self.cfg.episode_length - 1:
                    self.next_obses[k][:, self._idx] = v.clone().to(self.obses[k].device, dtype=self.obses[k].dtype)
                elif self._idx < self.cfg.episode_length - 1:
...
class ReplayBuffer:
    ...
    def add(self, episode: Episode):
        ep_len = len(episode)
        idxs = torch.arange(self.idx, self.idx + self.cfg.num_envs * ep_len) % self.capacity
        ...
        self._mask[idxs] = episode.masks.flatten(0, 1)
        ...
        mask = torch.arange(ep_len) > ep_len - self.cfg.horizon
        mask = torch.cat([mask] * self.cfg.num_envs)
        new_priorities = torch.full((ep_len  * self.cfg.num_envs,), max_priority, device=self.buffer_device)
        new_priorities[mask] = 0
        self._priorities[idxs] = new_priorities
        self._full = self._full or (self.idx + ep_len * self.cfg.num_envs > self.capacity)
        self.idx = (self.idx + ep_len * self.cfg.num_envs) % self.capacity
    ...
    def sample(self, bs=None):
        ...
        if not action.is_cuda:
            action, reward, mask, idxs, weights = (
                action.cuda(self.device),
                reward.cuda(self.device),
                mask.cuda(self.device),
