...
class TDMPC_DoraConfigs:
	...
	horizon_schedule: str = "1" # f"linear(1, {horizon}, 25000)"
...
def evaluate(test_env, agent, h1, step, video, action_repeat=1):
    ...
    for i in range(250): # int(test_env.max_episode_length // action_repeat)):
...
def train(args: argparse.Namespace) -> None:
    ...
    tdmpc_cfg.episode_length = int(env.max_episode_length // tdmpc_cfg.action_repeat)
    ...
    if fp is not None:
        ...
        step = episode_idx * int(env.max_episode_length // tdmpc_cfg.action_repeat)
